...
public class Story {
	...
	void introduction() { // introduces how to move and what the room look like and for example a room with a exit North will have the top line be empty indicating that you can move through it
	...
	void storyline() throws InterruptedException {
		if(Main.getCurrentRoom() == 0) { // if room is 0 its the first room teaching you how to move
			...
			if(!Main.map[Main.currentRoom].getRoomAlreadyExplored()){ // goes to the map array and see if the rooms alreadyexplored variable is true
			System.out.println("Wow! You found your first weapon! A sword!"); // Item.
			Item sword = new Item("Sword", 5, "weapon"); // makes a sword object, with the parameter name damage multiplier and item type
			...
			Main.map[Main.currentRoom].setRoomAlreadyExplored(true); // determines the room youre in right now and sets the value of alreadyExplored to true
			}
		} else if(Main.getCurrentRoom() == 4) {
			System.out.println("Continue moving right."); // directionals determine the main storyline you don't have to follow but its recommended
			...
			if(EnemyBattles.titan.getAlreadyFought() == false) { // gets the enemy alreadyfought variable and checks if true or false
